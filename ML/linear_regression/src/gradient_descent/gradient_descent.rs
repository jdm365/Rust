...
fn step(
    ...
    for i in 0..y_values.len() {
        b_gradient += -(2f32 / y_values.len() as f32) 
                      *  (y_values[i] - ((m_current * x_values[i]) + b_current));
        m_gradient += -(2f32 / y_values.len() as f32) * 
                      x_values[i] * (y_values[i] * ((m_current * x_values[i]) 
                      + b_current));
    }
